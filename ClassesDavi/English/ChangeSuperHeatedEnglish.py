...
class ChangeSuperHeatedPressureEnglish:
    ...
    def interpolDataFrame(self):
        data_frame1 = pd.read_excel(f'{self.fluid}_INGLES.xlsx', sheet_name=f'{self.fluid}_SUPER_AQ_{self.pressure1}'
                                                                            f'psi_INGLES')
        top_number1 = data_frame1.iloc[0, 0]
        bottom_number1 = data_frame1.iloc[-1, 0]
        rows1 = []
        for c1 in data_frame1['Temp(oF)']:
            rows1.append(c1)
        data_frame1.index = rows1[:]
        del data_frame1['Temp(oF)']
        columns1 = []
        for column in data_frame1:
            columns1.append(column)
        data_frame2 = pd.read_excel(f'{self.fluid}_INGLES.xlsx', sheet_name=f'{self.fluid}_SUPER_AQ_{self.pressure2}'
                                                                            f'psi_INGLES')
        top_number2 = data_frame2.iloc[0, 0]
        bottom_number2 = data_frame2.iloc[-1, 0]
        rows2 = []
        for c2 in data_frame2['Temp(oF)']:
            rows2.append(c2)
        data_frame2.index = rows2[:]
        del data_frame2['Temp(oF)']
        columns2 = []
        ...
        print(f'\033[0;33mAS PROPRIEDADES DESEJADAS SERAO INTERPOLADAS ENTRE AS TABELAS DE '
              f'{self.pressure1} psi E {self.pressure2} psi '
              f'MOSTRADAS A SEGUIR:\n\033[m')
        print(f'\033[0;33mTABELA DE {self.pressure1} psi\033[m:\n')
        print(data_frame1)
        print(f'\033[0;33mTABELA DE {self.chosen_pressure} psi\033[m:\n')
        print(interpolMe(self.chosen_pressure, self.pressure1, self.pressure2, data_frame1, data_frame2))
        print(f'\033[0;33m\nTABELA DE {self.pressure2} psi\033[m:\n')
        print(data_frame2)
