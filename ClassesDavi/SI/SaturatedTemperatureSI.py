...
class SaturatedDataFrameTemperatureSI:
    def __init__(self, fluid):
        self.fluid = fluid

    def research(self):
        data_frame = pd.read_excel(f'{self.fluid}_SI.xlsx', sheet_name=f'{self.fluid}_SAT_TEMP_SI')
    ...
    def get_enthalpy(self, input_temperature):
        data_frame = pd.read_excel(f'{self.fluid}_SI.xlsx', sheet_name=f'{self.fluid}_SAT_TEMP_SI')
        top_number = data_frame.iloc[0, 0]
        bottom_number = data_frame.iloc[-1, 0]
        rows = []
        ...
        while True:
            if input_temperature < top_number or input_temperature > bottom_number:
                ...
                return data_frame.loc[input_temperature]
            else:
                values_list = []
                for i, c1 in enumerate(rows):
                    if i > 0:
