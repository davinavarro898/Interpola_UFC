...
class SaturatedDataFramePressureSI:
    def __init__(self, fluid):
        self.fluid = fluid

    def research(self):
        data_frame = pd.read_excel(f'{self.fluid}_SI.xlsx', sheet_name=f'{self.fluid}_SAT_PRESS_SI')
        top_number = data_frame.iloc[0, 0]
        bottom_number = data_frame.iloc[-1, 0]
        rows = []
        for k in data_frame['Press(bar)']:
            rows.append(k)
        data_frame.index = rows
        del data_frame['Press(bar)']
        columns = []
        ...
        while True:
            try:
                input_pressure = float(input('\033[0;32mPRESSAO EM BAR:\033[m'))
            except:
